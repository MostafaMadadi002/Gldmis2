      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-4 lg:h-[calc(100vh-130px)] min-h-[600px]">
        ...
        <div className={`w-full lg:w-[40%] flex flex-col gap-3 printable-bill lg:sticky lg:top-4 lg:h-[calc(100vh-130px)] min-h-[500px] ${activeTab === 'cart' ? 'flex' : 'hidden lg:flex'}`}>
          ...
          <div className="bg-white dark:bg-dark-card rounded-2xl border border-kh-card/5 dark:border-dark-border shadow-sm flex flex-col overflow-hidden print:shadow-none print:border-none h-full">
            ...
            {/* Unified Printable Invoice (Realistic Luxury A4 Portrait) */}
                ...
                >
                  ...
                  <div className="flex justify-between items-start border-b-2 border-[#B8944A]/20 pb-6 mb-6">
                    ...
                    <div className="text-left space-y-1">
                      ...
                      <div className="text-sm font-black text-[#111827]">شماره: #{currentInvoiceNum}</div>
                      <div className="text-xs font-bold text-[#6B7280]">تاریخ: {new Date().toLocaleDateString('fa-IR')}</div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-10 mb-8">
                    <div className="flex gap-2 text-sm">
                      ...
                      <span className="font-bold text-[#111827]">{customerName || 'مشتری مهمان'}</span>
                    </div>
                    <div className="flex gap-2 text-sm text-left justify-end">
                      <span className="font-black text-[#B8944A]">فروشنده:</span>
                      <span className="font-bold text-[#111827]">{sellerDisplayName}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex-1">
                    <table className="w-full border-collapse">
                      <thead>
                        <tr className="bg-[#F8F7F3] border-y border-[#B8944A]/20">
                          ...
                          <th className="py-3 px-3 text-[10px] font-black text-[#6B7280] text-left w-32">قیمت واحد</th>
                        ...
                        {cart.map((item, index) => {
                          ...
                          return (
                            <tr key={index} className="text-xs">
                              ...
                              <td className="py-4 px-2 text-center font-black text-[#111827]">{item.selectedQuantity}</td>
                              <td className="py-4 px-2 text-center font-bold text-[#6B7280]">
                                {Number(item.selectedWeight || 0).toFixed(2)} <span className="text-[10px] text-slate-300">گرم</span>
                              </td>
                              <td className="py-4 px-3 text-left font-bold text-[#6B7280] font-mono">
                    ...
                    <div className="mt-8 flex justify-between items-start">
                      <div className="text-xs font-bold text-slate-400">
                        تعداد کل اجناس: <span className="text-[#111827]">{cart.reduce((sum, i) => sum + i.selectedQuantity, 0)}</span>
                      </div>
