      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-4 lg:h-[calc(100vh-130px)] min-h-[600px]">
        ...
        <div className={`w-full lg:w-[40%] flex flex-col gap-3 printable-bill lg:sticky lg:top-4 lg:h-[calc(100vh-130px)] min-h-[500px] ${activeTab === 'cart' ? 'flex' : 'hidden lg:flex'}`}>
          ...
          <div className="bg-white dark:bg-dark-card rounded-2xl border border-kh-card/5 dark:border-dark-border shadow-sm flex flex-col overflow-hidden print:shadow-none print:border-none h-full">
            ...
            {cart.length > 0 && (
              <div className={`printable-invoice bg-white text-black p-8 font-serif ${isRtl ? 'dir-rtl' : 'dir-ltr'}`} style={{ direction: isRtl ? 'rtl' : 'ltr' }}>
                {/* Header */}
                <div className="flex justify-between items-start border-b-4 border-black pb-6 mb-8">
                  <div className={`flex items-center gap-6 ${isRtl ? 'flex-row' : 'flex-row-reverse'}`}>
                    ...
                    <div className={isRtl ? 'text-right' : 'text-left'}>
                      ...
                      <p className="text-base font-bold text-gray-800">{shopInfo.description}</p>
                    </div>
                  </div>
                  <div className={isRtl ? 'text-left' : 'text-right'}>
                    <div className="text-2xl font-black mb-2 border-b-2 border-black pb-1 inline-block">{t('official_invoice')}</div>
                    <div className="text-sm font-bold text-gray-800">{t('invoice_no')}: <span className="font-black text-black">#{currentInvoiceNum}</span></div>
                    <div className="text-sm font-bold text-gray-800">{t('date')}: <span className="font-black text-black">{new Date().toLocaleDateString(language === 'en' ? 'en-US' : 'fa-IR')}</span></div>
                  </div>
                </div>

                {/* Info Section */}
                <div className="grid grid-cols-2 gap-10 mb-8 border-b-2 border-black pb-6">
                  <div className={`space-y-2 ${isRtl ? 'text-right' : 'text-left'}`}>
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-black text-gray-600">{t('customer_name')}:</span>
                      <span className="text-lg font-bold border-b border-black flex-1 px-2">{customerName || t('guest_customer')}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-black text-gray-600">{t('seller')}:</span>
                      <span className="text-lg font-bold border-b border-black flex-1 px-2">{user?.first_name ? `${user.first_name} ${user.last_name || ''}` : (user?.username || (language === 'en' ? 'System Admin' : 'مدیر سیستم'))}</span>
                    </div>
                  </div>
                  <div className={`space-y-1 ${isRtl ? 'text-left' : 'text-right'}`}>
                    <div className="text-sm font-bold">{t('address')}: {shopInfo.address}</div>
                    <div className="text-sm font-bold">{t('phone')}: {shopInfo.phone}</div>
                  </div>
                </div>
                ...
                <div className="mb-10">
                  {['gold', 'silver', 'jewelry', 'other'].map((mat) => {
                    ...
                    return (
                      <div key={mat} className="mb-6">
                        <div className={`bg-gray-100 px-4 py-1 border-black mb-2 ${isRtl ? 'border-r-4' : 'border-l-4'}`}>
                          <h3 className={`text-sm font-black uppercase ${isRtl ? 'text-right' : 'text-left'}`}>
                            {mat === 'gold' ? t('gold') : mat === 'silver' ? t('silver') : mat === 'jewelry' ? t('jewelry') : t('other')}
                          </h3>
                        </div>
                        <table className={`w-full border-collapse ${isRtl ? 'text-right' : 'text-left'}`}>
                          <thead>
                            <tr className={`border-b-2 border-black text-[10px] text-gray-500 uppercase ${isRtl ? 'text-right' : 'text-left'}`}>
                              <th className="py-2 px-2 font-black w-10">#</th>
                              <th className="py-2 px-2 font-black">{t('description')}</th>
                              <th className="py-2 px-2 font-black text-center">{t('code')}</th>
                              <th className="py-2 px-2 font-black text-center">{t('quantity')}</th>
                              <th className="py-2 px-2 font-black text-center">{t('weight')}/{t('carat')}</th>
                              <th className={`py-2 px-2 font-black ${isRtl ? 'text-left' : 'text-right'}`}>{t('unit_price')} ({t('afghani')})</th>
                              <th className={`py-2 px-2 font-black ${isRtl ? 'text-left' : 'text-right'}`}>{t('total_amount')}</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-gray-100">
                            {items.map((item, index) => {
                              ...
                              const totalPrice = unitPrice * item.selectedQuantity * (item.material === 'jewelry' ? 1 : item.selectedWeight);
                              ...
                              return (
                                <tr key={index} className="text-sm">
                                  ...
                                  <td className="py-3 px-2">
                                    ...
                                    <div className="text-[10px] font-bold text-gray-500">{item.carat || item.stoneType || ''}</div>
                                  </td>
                                  <td className="py-3 px-2 text-center font-mono text-xs">{item.code}</td>
                                  <td className="py-3 px-2 text-center font-black">{item.selectedQuantity}</td>
                                  <td className="py-3 px-2 text-center font-bold">
                                    {Number(item.selectedWeight || 0).toFixed(2)} {item.material === 'jewelry' ? t('carat') : t('gram_short')}
                                  </td>
                                  <td className={`py-3 px-2 font-bold text-gray-600 ${isRtl ? 'text-left' : 'text-right'}`}>
                                    {unitPrice.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                  </td>
                                  <td className={`py-3 px-2 font-black ${isRtl ? 'text-left' : 'text-right'}`}>
                ...
                {/* Totals & Signatures */}
                <div className={`flex justify-between items-start mt-auto pt-8 border-t-4 border-black ${isRtl ? 'flex-row' : 'flex-row-reverse'}`}>
                  <div className="grid grid-cols-2 gap-16 text-center pt-4">
                    <div className="w-48">
                      <p className="text-sm font-black border-b border-dashed border-black pb-1 mb-16">{t('seller_signature')}</p>
                      <p className="text-xs font-black">{user?.first_name ? `${user.first_name} ${user.last_name || ''}` : (user?.username || (language === 'en' ? 'System Admin' : 'مدیر سیستم'))}</p>
                    </div>
                    <div className="w-48">
                      <p className="text-sm font-black border-b border-dashed border-black pb-1 mb-16">{t('customer_signature')}</p>
                    </div>
                  </div>
                  
                  <div className={`bg-gray-100 p-6 rounded-2xl min-w-[300px] border-2 border-black ${isRtl ? 'text-right' : 'text-left'}`}>
                    <div className={`flex justify-between items-center mb-4 text-gray-700 ${isRtl ? 'flex-row' : 'flex-row-reverse'}`}>
                      <span className="text-sm font-black">{t('total')} {t('items')}:</span>
                      <span className="text-base font-black">{cart.reduce((sum, i) => sum + i.selectedQuantity, 0)} {t('product_name')}</span>
                    </div>
                    <div className={`flex justify-between items-center text-black ${isRtl ? 'flex-row' : 'flex-row-reverse'}`}>
                      <span className="text-lg font-black">{t('total_payable')}:</span>
                      <div className={isRtl ? 'text-right' : 'text-left'}>
                        <span className="text-3xl font-black">{totalAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                        <span className="text-sm font-black mx-1 text-gray-600">{t('afghani')}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {/* Footer / Terms */}
                <div className="mt-20 pt-8 border-t-2 border-black text-center">
                  <p className="text-sm font-black mb-2 leading-relaxed">{shopInfo.footerText}</p>
                  <div className="flex items-center justify-center gap-4 text-[10px] font-bold text-gray-500 mt-4 opacity-50">
                    <span>Khazana Jewelry Management System</span>
                    <span>•</span>
